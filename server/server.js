...
app.post('/api/v1/gemini/generate', authenticateOptionally, limitApiUsage, validateGeminiRequest, async (req, res) => {
    ...
    const { contents, generationConfig, safetySettings, systemInstruction, modelName, dice_roll_request } = req.body;
    ...
    try {
        ...
        let userInitiatedDiceResults = null; // Store the user's roll result separately
        ...
        if (dice_roll_request && Array.isArray(dice_roll_request) && dice_roll_request.length > 0) {
            ...
            userInitiatedDiceResults = diceResults; // Store the result specific to the user's action
        ...
        let lastAiDiceRollResults = null; // Track AI-initiated rolls separately
        const MAX_TURNS = 5;
        for (let turn = 0; turn < MAX_TURNS; turn++) {
            ...
            if (candidate.content?.parts?.[0]?.functionCall) {
                ...
                if (functionCall.name === 'rollDice') {
                    ...
                    lastAiDiceRollResults = diceResults; // Store AI's roll results
        ...
        // Prioritize sending back the user's roll result if it exists.
        // Otherwise, send back the AI's roll result if it made one.
